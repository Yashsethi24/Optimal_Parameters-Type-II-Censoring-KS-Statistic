...
kolcomp2<-function(N, brsize, itr, co, T0){
  ...
  for (i in 1:itr){
    ...
    bbmax[i,3]<-max(brabs[1:floor((co/N)*brsize)])
    bbmax[i,4]<-max(brabs[1:floor(rab*brsize)])
    bbmax[i,5]<-max(abs(sqrt((co-1)/N)*br+u*((co-1)/N-rab)*sqrt(N)), abs(co/N-rab))
...
brsize<-N*12                         # Grid size of Brownian bridge
fnitr<-2500                          # final  No. of iterations
...
for(i in 1 : fnitr) {
  brval<-kolcomp2(N, brsize, itr, co, T0 )       #Kolmogorv distribution values  5 values 
  kstestval<-Type(N,itr,co,T0)                   #computed values of KS statistic 3 values
  
  qtval[i,1]<-quantile(kstestval[,1],qt)       #KStest val for complete data 
  qtval[i,2]<-quantile(kstestval[,2],qt)      #KStest val for Type I data
  qtval[i,3]<-quantile(kstestval[,3],qt)       #KStest val for Type II data
  
  qtval[i,4]<-quantile(brval[,1],qt)           #BBridge val for Complete data
  qtval[i,5]<-quantile(brval[,2],qt)           #BBridge val for Type I data
  qtval[i,6]<-quantile(brval[,3],qt)           #BBridge val for Type II data using co /n 
  qtval[i,7]<-quantile(brval[,4],qt)          #BBridge val for Type II data using beta
  qtval[i,8]<-quantile(brval[,5],qt)          #BBridge val for Type II data using func
  ...
  ksqt1<-(ks.test(kstestval[,2],brval[,2],alternative = "two.sided"))
  ksdiff1[i,]<-c(ksqt0$statistic, ksqt0$p.value)
  
  ksqt2a<-(ks.test(kstestval[,3],brval[,3],alternative = "two.sided"))
  ksdiff2a[i,]<-c(ksqt2a$statistic, ksqt2a$p.value)
  
  ksqt2b<-(ks.test(kstestval[,3],brval[,4],alternative = "two.sided"))
  ksdiff2b[i,]<-c(ksqt2b$statistic, ksqt2b$p.value)
  
  ksqt2c<-(ks.test(kstestval[,3],brval[,5],alternative = "two.sided"))
  ksdiff2c[i,]<-c(ksqt2c$statistic, ksqt2c$p.value)
  
 cat(i, date() ,"\n ")
}

write.csv (qtval, "qtval1.csv"   )

qtval<-read.csv("qtval1.csv",header=T,sep=",")

  qtdiff0[,1 ]<-qtval[,5]-qtval[,2]
  qtdiff1[,1 ]<-qtval[,6]-qtval[,3]
  qtdiff2a[,1 ]<-qtval[,7]-qtval[,4]
  qtdiff2b[,1 ]<-qtval[,8]-qtval[,4]
  qtdiff2c[,1 ]<-qtval[,9]-qtval[,4]
  
  
plot(ecdf(qtdiff2a[,1]),col=2)
lines(ecdf(qtdiff2b[,1]),col=3)
lines(ecdf(qtdiff2c[,1]),col=4)
abline(v=0)

cat(mean((qtdiff2a[,1])^2),"\n")
cat(mean((qtdiff2b[,1])^2),"\n")
cat(mean((qtdiff2c[,1])^2),"\n")
